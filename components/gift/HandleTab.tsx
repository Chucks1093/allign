...
export default function HandleTab({ address, holdings, selected, setSelected, stockContracts }: Props) {
  ...
  const canSend = !!handle && !!selected && parseFloat(amount) > 0 && !sending && !!walletClient;
    ...
    return (
      <div className="max-w-xl mx-auto space-y-6 font-[var(--font-manrope)]">
        <div className="space-y-1">
          ...
          <p className="text-white/40 text-sm">Share this link with <span className="text-white">@{handle}</span> — they'll claim it at /claim.</p>
        </div>
        ...
        <button onClick={() => { setClaimLink(null); setHandle(""); setAmount(""); }}
  ...
  return (
    <div className="max-w-xl mx-auto space-y-4 font-[var(--font-manrope)]">
      ...
      <div className="grid grid-cols-4 gap-3">
        {PLATFORMS.map((p) => (
          <button key={p.id} onClick={() => setPlatform(p.id)}
            className={`rounded-xl p-4 flex flex-col items-center justify-center gap-3 aspect-square transition-all cursor-pointer ${
              platform === p.id
      ...
      {/* Handle input */}
      <div className="bg-[#1a1a1a] border rounded-xl px-4 py-4 flex items-center gap-3 transition-colors"
      ...
      <button onClick={handleSend} disabled={!canSend}
        className="w-full py-4 rounded-xl bg-white hover:bg-white/90 text-black font-bold text-base transition-colors cursor-pointer disabled:opacity-30 disabled:cursor-not-allowed flex items-center justify-center gap-2">
        {sending ? <><Loader2 size={16} className="animate-spin" /> {statusMsg || "Processing…"}</> : "Send it"}
      </button>
    </div>
  );
}
